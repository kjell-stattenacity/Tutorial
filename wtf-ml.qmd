...
library(data.table)
library(sfdf)   # from github.com/topepo/sfd
library(patchwork)
library(tidymodels)
...
#| out-width: "80%"
#| fig-align: center
#| fig-cap: "PCA plots for each pre-processing method."
...
The operations that you apply to the predictors before the model are at least as important as which supervised model that you use. _Feature engineering_ is the process of representing the predictor data in a way that makes the model have to work the least to be effective.
:::
...
#| warning: false
#| cache: true
#| fig-cap: "An example spa ce-filling design for two tuning parameters in a Cubist model"
...
@fig-pls-rf-tune-profiles (right) displays the tuning parameter profile for the RF model.  We can see from this figure that as the value of the $m_{try}$ is fairly irrelevant so long as very fewer predictors are randomly sampled.  Also, the predictor set with the best RMSE is the same as the set identified using PLS.  However, the optimal RF model (RMSE = `r format(min(best_rf$mean), digits = 3)`) performs poorly compared to the optimal PLS model (RMSE = `r format(min(best_pls$mean), digits = 3)`).  
...
all_svm_metrics %>% 
  ...
  ggplot(aes(.iter, mean, col = Pre, group = Pre)) + 
  geom_point() + 
  geom_line() + 
  theme(legend.position = "top") +
  scale_color_brewer(palette = "Set2") +
...
all_best %>% 
  ggplot(aes(Pre, RMSE, col = Model, group = Model, pch = Model)) + 
  geom_point(cex = 2) +
  geom_line() + 
