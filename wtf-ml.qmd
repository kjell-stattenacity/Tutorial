...
Whether a model is a "machine learning model" depends on how it will be used rather than the mathematical defintion of the technique. If the goal of an analysis is to make the most accurate prediction possible, it's fair to use the ML label. 
:::
...
You probably don't need a complex black-box machine learning model (e.g. deep neural networks, large emsemble models, etc.). 
:::
...
TODO where to we introduce the (x, x, x) notation? 
...
worst_resid_ids <- 
  all_pred %>% 
  mutate(abs_resid = abs(.pred - concentration)) %>% 
  slice_max(abs_resid, n = 5) %>% 
  count(sample_id) %>% 
  arrange(desc(n)) %>% 
  pluck("sample_id")
...
all_pred <- 
  ...
  mutate(
    ...
    sample_note = ifelse(sample_id == worst_resid_ids[1], paste(worst_resid_ids[1]), sample_note),
    sample_note = ifelse(sample_id == worst_resid_ids[2], paste(worst_resid_ids[2]), sample_note)
  )
all_pred_most <- filter(all_pred, sample_note == "everything")
all_pred_34 <- filter(all_pred, sample_note == "34")
all_pred_7 <- filter(all_pred, sample_note == "7")
```
...
The samples with the largest residuals were `r knitr::combine_words(worst_resid_ids)`. These are shown in @fig-obs-pred-cv-comparison with red triangles and blue squares, respectively. 
...
all_pred_most %>% 
  ...
  geom_point(data = all_pred_7, col = "blue", pch = 15, cex = 2) + 
...
This tutorial was intended to provide readers with a realistic worked example of machine learning with a CMC application and also to demonstrate that there is still a fair amount of art in this particular area of science. The content in most of our WTF items is borne out of experience. Our advice for practitioners just starting out is to read as much literature as possible, try many different approaches, and be very dogmatic about thoroughly validating your results.
